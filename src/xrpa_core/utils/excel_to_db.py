...
from xrpa_core.db.models import ScmBase
...
class ExcelToDatabase:
    ...
    def import_excel(
        ...
        Args:
            ...
            field_mapping: 字段映射关系。
                支持 {模型属性名: excel列名}
                也支持 {模型属性名: "列A|列B"} 或 {模型属性名: ["列A", "列B"]}
            sheet_name: Excel工作表名称或索引（csv文件会忽略该参数）
            custom_fields_handler: 自定义字段处理函数，接收一行数据(pd.Series)，返回额外字段的字典
            start_row: 数据起始行(从0开始)，用于跳过前置说明行
            batch_size: 批量插入的大小
            update_on_conflict: 是否在主键冲突时更新，默认False（跳过）
            conflict_keys: 冲突判定键（默认使用主键）
            config: 导入配置(包含字段映射和清洗器)
        Returns:
            包含导入统计信息的字典
        """
        (
            ...
            keep_conflict_keys=True,
        )

        logger.info("构建数据记录: {}", excel_path)
    ...
    def import_excel_bulk(
        ...
        """
        使用bulk_insert_mappings批量导入（支持xlsx/csv，性能更高，但不触发ORM事件）
        ...
        Args:
            ...
            field_mapping: 字段映射关系。
                ...
                也支持 {模型属性名: "列A|列B"} 或 {模型属性名: ["列A", "列B"]}
            sheet_name: Excel工作表名称或索引（csv文件会忽略该参数）
            custom_fields_handler: 自定义字段处理函数
            start_row: 数据起始行(从0开始)，用于跳过前置说明行
            batch_size: 批量插入的大小
            config: 导入配置(包含字段映射和清洗器)
        ...
        (
            ...
            keep_conflict_keys=False,
        )

        proxy_config = ExcelImportConfig(
            field_mapping=resolved_field_mapping,
            field_cleaners=field_cleaners,
            custom_fields_handler=custom_fields_handler,
            sheet_name=sheet_name,
    ...
    def _resolve_import_options(
        self,
        field_mapping: dict[str, str | list[str]] | None,
        sheet_name: str | int | None,
        custom_fields_handler: Callable[[pd.Series], dict[str, Any]] | None,
        start_row: int | None,
        conflict_keys: list[str] | None,
        config: ExcelImportConfig | None,
        default_sheet_name: str | int,
        keep_conflict_keys: bool,
    ) -> tuple[
        ...
        resolved_conflict_keys = conflict_keys if keep_conflict_keys else None

        if config:
            if field_mapping is None:
                field_mapping = config.field_mapping
            field_cleaners = config.field_cleaners
            if custom_fields_handler is None:
                custom_fields_handler = config.custom_fields_handler
            if sheet_name is None:
                sheet_name = config.sheet_name
            if start_row is None:
                start_row = config.start_row
            if keep_conflict_keys and resolved_conflict_keys is None:
                resolved_conflict_keys = config.conflict_keys

        if field_mapping is None:
            raise ValueError("field_mapping is required")
        if sheet_name is None:
    ...
    def _build_records(
        ...
        # 解析映射关系（支持多个候选列名）
        resolved_field_mapping = self._resolve_field_mapping(field_mapping, df.columns)
    ...
    def _resolve_field_mapping(
        ...
        if missing_groups:
            raise ValueError(f"Excel中缺少以下列（每组至少命中一个）: {missing_groups}")
    ...
    def _parse_mapping_candidates(
        ...
        if isinstance(source_cols, str):
            ...
            raise ValueError("field_mapping 的value必须是str或list[str]")
                ...
                try:
                    ...
                    if not unique_warned:
                        ...
                        if "unique" in msg:
                            ...
                            logger.warning("插入记录时发生完整性错误，已跳过")
